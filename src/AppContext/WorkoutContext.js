...
export class WorkoutProvider extends Component {
  ...
  InsertNewRoutine = () => {
    let currentRoutine = {
      name: "Enter a name for your workout routine",
      exercises: [],
    };
    this.setState({
      currentRoutine,
  ...
  SaveSetValues = (e, workoutIndex, exerciseIndex, setIndex) => {
    ...
    placeholder = placeholder === "mins" ? "Weight" : placeholder;
    placeholder = placeholder === "cals" ? "Reps" : placeholder;
    if (workoutIndex !== "") {
      newState = workouts;
      stateItem = "workouts";
      newState[workoutIndex].exercises[exerciseIndex].sets[setIndex][
        placeholder
      ] = value;
    } else {
      newState = { ...currentRoutine };
  ...
  SaveWorkout = (workoutIndex) => {
    ...
    let requestObject =
      workoutIndex !== ""
        ...
        : { ...currentRoutine, date: workoutDate };
    let confirm = window.confirm("Would you like to save this workout?");
    if (confirm) {
      ...
      alert(`This workout is saved for ${workoutDate}`);
      this.GetWorkouts();
    }
  };
