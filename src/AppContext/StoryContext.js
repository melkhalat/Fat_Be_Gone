...
export class StoryProvider extends Component {
  ...
  ClearSubmissionForm = () => {
    ...
    document.getElementById("StoryMediaLabel").textContent =
  ...
  DeleteStory = (story_id, file) => {
    ...
    if (confirm) {
      ...
      let fileParam = file
        ...
        ? `&public_id=${file.public_id}&resource_type=${file.resource_type}`
  ...
  PutFileInLabel = (e) => {
    ...
    label.childNodes[0].textContent =
      ...
      (newFile || {}).name || "Insert Image or Video";
    ...
    if (newFile) {
      ...
      let view =
        ...
        e.target.parentElement.id === "StoryModalBtnWrapper"

          ? "modal"

          : "submission";

      document.getElementById("StorySubmitMediaDiv").style.display =

        view === "submission" ? "flex" : "none";
  ...
  CheckFileSize = (file) => {
    ...
    if (fileSize > 10 && fileSize < 50) {
      ...
      alert(
        ...
        "This file is pretty large so it will take a bit of time to be uploaded!"
